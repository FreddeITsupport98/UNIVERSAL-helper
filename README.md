...
<a id="downloader-prefetch-error-classification"></a>
### Downloader prefetch error classification (`error:solver:1` / libdnf5 sandbox)
- The dashboard's **"Downloader prefetch error"** Notification Center popup with `Status: error:solver:1` / `Detail: Solver/download error (rc=1)` / `Incident: inc-downloader-solver-1` is no longer raised for failures that are not actually solver/conflict errors.
- The most common cause on Fedora hosts is `dnf system-upgrade download --refresh --releasever=N -y` failing inside the `zypper-autodownload.service` sandbox with: `filesystem error: cannot create directories: Read-only file system [/usr/lib/sysimage/libdnf5/offline]`. That is a `ReadWritePaths=` configuration problem (dnf5 needs to stage the offline upgrade under `/usr/lib/sysimage/libdnf5/offline/`), not a solver problem.
- Self-heal on existing installs:
  - `sudo zypper-auto-helper --verify` will detect a stale `ReadWritePaths=` line on the deployed unit, rewrite it to include `-/usr/lib/sysimage/libdnf5 -/usr/lib/sysimage/libdnf5/offline`, run `systemctl daemon-reload`, `systemctl reset-failed zypper-autodownload.service`, and restart the timer.
  - `sudo systemctl start zypper-autodownload.service` to clear the stale `error:solver:1` from `/var/log/zypper-auto/download-status.txt`.
- The downloader script now classifies a non-zero rc into the most accurate status before writing it to `download-status.txt`:
  - `complete:0:0` if stderr is empty or matches a per-PM "Nothing to do" / "All packages are up to date" / `0 upgraded, 0 newly installed, 0 to remove` style message.
  - `error:network` if stderr matches DNS/connect/repo-metadata fetch markers (e.g. `could not resolve host`, `failed to retrieve new repository metadata`).
  - `error:repo` if stderr matches `Read-only file system|EROFS|sandbox.*read-only|cannot create directories: Read-only` (the message in `Notifications` will explicitly mention `ReadWritePaths=` so you know where to look).
  - `error:solver:RC` only when the captured stderr actually contains real conflict/solver markers (`conflict|conflicting requests|nothing provides|has inferior architecture|unresolvable|problem with installed package|solver problem|cannot install both|requires.*but it cannot be installed`).
- The downloader event log (`/var/log/zypper-auto/downloader-events.log`) now carries an additional `error_kind=...` taxonomy field (`benign|network|repo|readonly_fs|solver|none`) on every `DOWNLOADER_EVENT` line, so the AI Smart Report's repair-plan catalog (`/api/ai/smart-report`) can match the new `downloader-readonly-fs` incident kind directly without re-parsing the free-form `status=...` string. The legacy `status=` and `code=` fields stay intact for backward compatibility, and existing log scrapers keep working.
- The shell-side classifier widens the read-only sandbox detector with extra dnf5/libdnf5 wording variants (`mkstemp.*read-only`, `mkdir.*read-only`, `failed to create.*read-only`, `unable to create.*read-only`, `permission denied.*read-only`, the lower-case `read-only filesystem` spelling, and the `errno=30` EROFS errno) so future libdnf5 plugin output keeps being classified correctly. The original `read-only file system|EROFS|cannot create directories: Read-only` markers stay first for backward compatibility.
- WebUI sub-class: read-only sandbox failures now surface as `error:repo:readonly_fs` (instead of the bare `error:repo`). The Notification Center body explicitly mentions `ReadWritePaths=` and the libdnf5 offline staging dir hint, and the bell dedupes on the new `inc-downloader-repo-readonly-fs` incident id while existing `inc-downloader-network` / `inc-downloader-repo` / `inc-downloader-solver-RC` ids stay byte-identical.
...
- **Unreleased (next build):**
  - 🚀 **IMPROVED:** Distro-upgrade rocket button 🚀 now glows green for **all** fixed-cycle distros (Ubuntu, Mint, Debian, Fedora, openSUSE Leap, RHEL…) when a major version upgrade is available — not just when individual package updates are pending. The `applyLiveData()` live-polling loop and the `znhDistroUpgradeRender()` banner renderer both contribute to the glow; the rocket clears when the banner is dismissed. Clicking 🚀 when a distro upgrade is actionable and no package updates are pending now opens the distro-upgrade wizard directly instead of the regular package wizard.
  - 🔢 **FIXED:** Ubuntu distro-upgrade banner no longer shows `found.` as the target version. The version is now extracted with 4 fallback methods: (1) `grep -oE '[0-9]{2}\.[0-9]{2}'` directly from `do-release-upgrade -c` output, (2) codename-to-version map (`focal=20.04`, `jammy=22.04`, `noble=24.04`, `plucky=25.04`, `regal=26.04`, …), (3) Ubuntu meta-release feed fetched via `curl` (respects LTS-only prompt setting), (4) April/October release-cycle math from the current `VERSION_ID`.
  - 🔢 **NEW:** Debian upgrade banner now shows the correct next stable version. `znh_distro_upgrade_check_debian()` maps the current codename: `bookworm → 13 (trixie)`, `trixie → 14 (forky)`, etc. The detected target is written to `distro-upgrade.json` so the WebUI wizard shows e.g. `Debian 12 → 13 (trixie) — manual upgrade required`.
  - 🔢 **NEW:** openSUSE Leap upgrade banner now shows the expected next version. `znh_distro_upgrade_check_leap()` probes zypper repos for the next `major.minor+1` release and falls back to arithmetic, showing e.g. `openSUSE Leap 15.6 → 15.7`.
  - 🔢 **NEW:** RHEL/CentOS/Rocky/Alma upgrade banner now shows the next major version computed inline (e.g. `9.x → 10`, `10.x → 11`) using `VERSION_ID` arithmetic.
  - 🔧 **IMPROVED:** Rocket Wizard `_ruRenderDistroUpgrade()` now shows a meaningful `🔧 Distro current → target — manual upgrade required` view for Debian/Leap/RHEL (status=`manual`) when a target version is known, replacing the generic "no upgrade available" message. An "Open upstream guide" button is included.
  - 🐛 **FIXED:** `scripts/syntax-check.sh` shellcheck SC2181 warning at line ~37199 suppressed with an inline `# shellcheck disable=SC2181` comment (the `$?` check follows a multi-KB heredoc and cannot be restructured without high risk).
  - 🧪 **IMPROVED:** `regressions/test_distro_upgrade_detect_regression.sh` updated with 8 new assertions (checks 19-26) covering the new Ubuntu codename helpers, the 4-method extraction, Debian/Leap target detection, RHEL next-major arithmetic, state file target-passing, URL sanitization, and the JS manual-status wizard view.
  - WebUI Active Modules Snap badge now reflects actual Snap Store reachability (`feat_snap_healthy` set only when `snap debug connectivity` succeeds; yellow when unreachable instead of falsely green)
  - `--setup-SF` Snap Store reachability check switched from `curl` API ping to `snap debug connectivity`; KDE Discover removal now gated on BOTH Flathub and Snap Store being reachable
  - _TBD_
  - 🔇 **FIXED:** `zypper-run-install` no longer prints "PM runtime helper not found; attempting auto-deploy..." or "PM runtime helper not available; using built-in cross-distro fallback." on the terminal when the system PM runtime file is missing. Both messages are now log()-only. A user-local PM runtime cache ($HOME/.local/share/zypper-notify/pm-runtime-cache.sh) is checked before the auto-deploy attempt (no root required), and on first fallback activation the inline stubs are written to that cache via declare -f so subsequent runs load from cache without any auto-deploy overhead.
  - 📝 **CHANGED:** the early non-zypper boot notice in `UNI-auto.sh` (around line 95) no longer claims "the update engine remains zypper-focused". The new notice text reflects that apt/dnf/pacman are wired through the shared package-manager runtime (refresh/preview/install/view-changes/notifier), the PM-aware Rocket WebUI, the classifier-based downloader prefetch, and the filesystem/PM-aware Snapper relevance gate; only zypper-only deep-repair paths (turbo tuning, stale zypp cache sweep, zypp lock final pass) remain gated as zypper-only and are skipped with reason logs on other backends.
  - 🧪 **NEW:** added focused static regression `regressions/test_notifier_snapper_relevance_regression.sh` that locks the cross-distro Snapper-relevance gate contract: it asserts `_root_filesystem_type()` exists and probes `findmnt -n -o FSTYPE /` with a `/proc/mounts` fallback; `_snapper_is_relevant()` exists and returns true only when root is btrfs OR detection failed and `SYSTEM_PKG_MANAGER == "zypper"`; `check_snapshots()` calls `_snapper_is_relevant()` early and short-circuits with `(True, "")` plus the debug log line documenting the skip; and the notifier still imports `shutil` (required by `_root_filesystem_type` for `shutil.which("findmnt")`). The test prints a `FAIL SUMMARY (N)` block on failure as required.
  - 🌐 **FIXED:** desktop notifier `Snapper not installed` warning is no longer surfaced on cross-distro hosts where Snapper is not relevant. The embedded Python notifier (`zypper-notify-updater.py`) now performs a filesystem + package-manager relevance check via two new helpers (`_root_filesystem_type()` parses `findmnt -n -o FSTYPE /` with a `/proc/mounts` fallback, and `_snapper_is_relevant()` returns true only when the root filesystem is btrfs OR detection failed and the package manager is `zypper`). When Snapper is not relevant on the host, `check_snapshots()` short-circuits with `(True, "")` and the existing `if snapshot_msg:` guard at the call site silently suppresses the `⚠️ Snapper not installed` line in the **Updates Ready to Install** notification. Tumbleweed/Leap btrfs hosts where the original Snapper card is meaningful keep their existing behavior unchanged. Also realigned the static guard `regressions/test_cross_distro_package_manager_regression.sh` to require the cross-distro downloader prefetch path to call `znh_downloader_classify_download_failure "prefetch download" "${ZYP_RET}" "${DL_ERR_FILE}"` instead of the legacy blanket `znh_downloader_write_status "error:solver:$ZYP_RET"` write (the new behavior is already enforced by `regressions/test_downloader_prefetch_error_classification_regression.sh`). Verified with `bash -n UNI-auto.sh`, `python3 -m py_compile` on the extracted notifier, and the cross-distro + classifier regressions pass.
  - 🚀 **NEW:** Distro-upgrade Apply via Rocket now shows live download progress and a one-click "Reboot now to finish upgrade" path. The Rocket Update Manager wizard keeps the overlay open after kicking off `sudo zypper-auto-helper --distro-upgrade apply --yes` and renders the same big banner used by the regular Rocket update flow (stage + percent + live log), driven by the existing SSE quick-action stream with a polling fallback. When the staged download finishes successfully, a result view surfaces the family-specific finishing command (Fedora/Nobara: `sudo dnf system-upgrade reboot`) plus three buttons: **Reboot now to finish upgrade** (Fedora-only one-click reboot via the new `distro-upgrade-reboot` quick-action with confirmation phrase `REBOOTUPGRADE`), **Copy reboot command**, and **Open terminal (paste)**. Helper CLI gained `znh_distro_upgrade_run_finish` and a `--distro-upgrade finish [--yes]` (alias `reboot`) subcommand that runs `dnf|dnf5 system-upgrade reboot` on Fedora/Nobara and prints reboot guidance for Ubuntu/Mint/Debian/Leap/RHEL while short-circuiting on rolling distros. The reboot quick-action requires root, refuses without `--yes` in non-interactive mode (no TTY), and is wired into the WebUI quick-action allowlist with explicit explain/warning copy stating it WILL reboot the machine. Extended `regressions/test_distro_upgrade_detect_regression.sh` to guard the new bash helper, dispatcher branch, quick-action entry + `REBOOTUPGRADE` confirmation phrase, and the WebUI progress/reboot wiring (function names, SSE stream usage).
  - 🚀 **NEW:** Release-cycle aware **distro-upgrade detection** is now built into `UNI-auto.sh`. The helper classifies the running distro into `rolling` (Tumbleweed/Slowroll, Arch/Manjaro/EndeavourOS/Garuda/Artix/CachyOS/Siduction/Gentoo/Void/NixOS/Chimera/KaOS — left alone, no distro-upgrade event), `fixed` (Fedora ~6 month cycle; Ubuntu/Pop/Elementary/Neon/Kali/Zorin; Linux Mint/LMDE; Debian/Raspbian/Devuan; openSUSE Leap/SLES; RHEL/Rocky/Alma/Oracle), or `unknown`. The Fedora probe runs `dnf|dnf5 repoquery --releasever=N+1..N+3 fedora-release` so a newer Fedora release surfaces automatically; Ubuntu/Mint reuse `do-release-upgrade -c`; Debian/Leap/RHEL surface a `manual` state with the canonical upstream guide URLs. Each run writes `/var/lib/zypper-auto/distro-upgrade.json` (schema 1) with `status` / `release_model` / `distro_family` / `distro_id` / `distro_name` / `current_version` / `target_version` / `reason` / `checked_at` / `package_manager` so the WebUI dashboard and notifier can render a `Ready to install` card without re-querying the package manager.
  - 🚀 **NEW:** Three new CLI flags expose the distro-upgrade flow on `zypper-auto-helper`: `--check-distro-upgrade` (alias `--distro-upgrade-check`) refreshes the state, prints a status report, and emits a `notify-send` desktop notification (`-i system-software-update -u normal -t 20000`) addressed to `${SUDO_USER}` plus a journald `logger -t zypper-auto-helper-distro-upgrade` line; `--distro-upgrade-status` prints the current state without contacting the network; and `--distro-upgrade [status|check|apply [--yes]]` initiates the upgrade. The `apply` path requires root, asks for explicit `YES` confirmation (or `--yes`), runs `dnf system-upgrade download --refresh --releasever=<target>` for Fedora and `do-release-upgrade` for Ubuntu/Mint, and instructs the user to run `sudo dnf system-upgrade reboot` afterwards. Rolling distros short-circuit to a friendly "no distro upgrade event needed" message instead of trying to upgrade.
  - 🧹 **IMPROVED:** Uninstaller (`--uninstall-zypper`) now lists `/var/lib/zypper-auto/distro-upgrade.json` in its dry-run output and removes it during cleanup alongside the existing `dashboard-history.sqlite3` / `self-update-state.json` / `snapper-auto-disabled.intent` artifacts so `--uninstall-zypper` leaves no orphaned distro-upgrade state behind.
  - 🧪 **NEW:** Added focused static regression `regressions/test_distro_upgrade_detect_regression.sh` that asserts every new helper function exists, the classifier covers rolling vs fixed-cycle distro IDs, the Fedora probe uses `repoquery --releasever="${candidate}" fedora-release`, the Ubuntu probe uses `do-release-upgrade -c`, the state JSON contract emits all expected keys, the notification helper invokes `notify-send` with the `system-software-update` icon and the `zypper-auto-helper-distro-upgrade` journald tag, the early option allowlist exposes all four new flags, the main dispatcher contains `--check-distro-upgrade` / `--distro-upgrade-status` / `--distro-upgrade` branches that forward to `znh_distro_upgrade_cli_dispatch`, the `--help` text exposes the new commands, and the uninstaller dry-run + cleanup paths both reference `distro-upgrade.json`. The test prints a `FAIL SUMMARY (N)` block on failure as required by repo regression conventions.
  - 🧪 **IMPROVED:** central regression runner preflight is now green without `RUNNER_SKIP_SHELLCHECK=1`. Suppressed an info-level shellcheck false positive (SC2016) in `regressions/test_dash_bg_regression.sh:106` where the assertion deliberately greps `UNI-auto.sh` for a literal unexpanded `$SUDO_USER_HOME` reference. Full suite (`run_regression_suite.sh`) now runs 34 shell + 8 Python regressions to PASS with the default preflight enabled (Playwright runtime tests still skipped when Playwright is not installed).
  - 🐛 **FIXED:** dashboard-blocking JavaScript syntax error in the WebUI Settings drawer (`Uncaught SyntaxError: Unexpected token '{'` at `status.html?live=1` ~line 7596) caused by a malformed `SETTINGS_FIELDS` entry for `ZNH_DIAG_MAX_SERVICE_LOGS` in `UNI-auto.sh:19289` (missing `type`, `label`, `advanced`, `help`, and the closing `},`). Restored the full entry definition so the row renders next to `ZNH_LIVE_LOGS_MAX_SERVICE_LOGS`, the bootstrap watchdog no longer reports "Main dashboard script did not start (possible syntax error)", and the rest of the dashboard JS (live polling, settings autosave, snapper overlays, etc.) loads again. Schema and config defaults for the same key were already correct at `UNI-auto.sh:2866` and `UNI-auto.sh:2025` / `UNI-auto.sh:9028-9034` and remain unchanged. Verified with `bash -n UNI-auto.sh` plus `scripts/syntax-check.sh` and a brace/bracket balance check on the SETTINGS_FIELDS array. Regenerate the dashboard (`sudo zypper-auto-helper --dashboard`, then `zypper-auto-helper --dash-open`) so the on-disk `~/.local/share/zypper-notify/status.html` picks up the fix.
  - Dashboard `status-data.json` generation is now writer-hardened: it prefers atomic Python `json.dump`/`os.replace` serialization for status payloads (including large `last_install_tail` content), with the existing shell writer kept as a fallback when `python3` is unavailable.
  - Extended static regression `regressions/test_webui_poll_parse_recovery_regression.sh` to also guard status-data writer-side hardening contracts (python serializer path, atomic replace, and fallback guard wiring) in addition to pollLive parse-recovery checks.
  ...
  - Failed-unit classifier patterns are now config-driven with dashboard/settings API wiring (`VERIFY_FAILED_UNITS_CRITICAL_GLOBS`, `VERIFY_FAILED_UNITS_CRITICAL_REGEX`, `VERIFY_FAILED_UNITS_NOISE_GLOBS`, `VERIFY_FAILED_UNITS_NOISE_REGEX`), so critical/noise matching can be tuned without editing verification logic.
  - Added runtime regression `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` that executes extracted embedded Dashboard API helpers and validates Rocket PM-aware recovery/progress behavior (legacy status PM fallback, zypper-only prettifier gating, and PM-specific progress stage mapping used by SSE updates).
  - Extended runtime regression `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` with handler-level `/api/events/job` stream checks that execute extracted embedded `Handler` SSE logic end-to-end and assert `reset/append/done` sequencing, PM-aware append stage/progress payloads, and zypper-only prettifier gating for streamed append chunks.
  - Extended the same runtime regression with a non-Rocket `self-update` `/api/events/job` handler-level case that validates end-to-end `reset/append/done` sequencing, self-update progress-stage parsing (`Downloading`), and empty `package_manager` SSE payload contract for non-`system-dup` streams.
  - Extended that `self-update` SSE runtime coverage with an explicit failure-path done-event case (`rc != 0`) to assert `Failed` stage mapping and non-zero done payload propagation for `/api/events/job` non-`system-dup` streams.
  - Refactored `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` SSE handler-runtime assertions into shared helper functions (`collect_sse_events`, `validate_sse_sequence_and_extract_payloads`, `assert_sse_done_payload_contract`) so both `system-dup` and `self-update` stream cases reuse one sequence/done-contract assertion path.
  - Further refactored `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` by centralizing duplicated `/api/events/job` handler bootstrap and lifecycle logic into shared helper functions (`launch_sse_handler`, `wait_for_sse_thread_termination`) reused by both `system-dup` and `self-update` runtime cases.
  - Further refactored `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` by centralizing self-update append-event completion gating into a shared helper (`wait_for_sse_event_in_stream`) so append-wait polling/timeouts are reusable and consistent across future SSE runtime cases.
  - Further refactored `regressions/test_rocket_pm_runtime_recovery_sse_regression.sh` by extracting shared append payload contract checks into `assert_sse_append_payload_contract` and converting `system-dup`/`self-update` runtime invocations to table-driven case loops, reducing duplicated assertion/case wiring while preserving existing SSE coverage.
  - Installer cleanup now guards legacy user pycache sweeps behind a `~/.local/bin` existence check, so first-run/user-cleanup paths no longer emit false `FAILED` lines when that directory is absent.
  - Stale-config warning output now includes explicit per-key descriptions for `LOCK_REMINDER_ENABLED`, `NO_UPDATES_REMINDER_REPEAT_ENABLED`, `UPDATES_READY_REMINDER_REPEAT_ENABLED`, and `LOG_FOLDER_OPENER` instead of generic `(no description available)` entries.
  - Added focused static regression `regressions/test_cleanup_guard_and_config_warning_descriptions_regression.sh` to guard both the legacy pycache-cleanup skip behavior and the missing-key description coverage.
