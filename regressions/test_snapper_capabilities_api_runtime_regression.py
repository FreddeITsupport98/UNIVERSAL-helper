...
class SnapperCapabilitiesApiRuntimeRegressionTest(unittest.TestCase):
    ...
    def _invoke_caps_with_mocks(
        self,
        *,
        root_fstype: str,
        snapper_installed: bool,
        snapper_root_config: bool,
        bls_present: bool,
        grub_present: bool,
        grub_bls_mode: bool,
        loader_conf_present: bool,
    ) -> tuple[int, dict]:
        ...
        def _fake_run(cmd, **_kwargs):  # noqa: ANN001
            ...
            if c[:5] == ["findmnt", "-n", "-o", "FSTYPE", "/"]:
                return _CP(f"{root_fstype}\n", 0)
            return _CP("", 0)

        def _fake_which(name: str):
            n = str(name or "").strip()
            if n == "snapper":
                return "/usr/bin/snapper" if snapper_installed else None
            return None
        ...
        _orig_glob = self.ns["glob"].glob
        _orig_isdir = self.ns["os"].path.isdir
        _orig_exists = self.ns["os"].path.exists
        _orig_isfile = self.ns["os"].path.isfile
        _orig_getsize = self.ns["os"].path.getsize

        def _fake_open(path, mode="r", *args, **kwargs):  # noqa: ANN001
            p = str(path or "")
            if p == grub_cfg and "r" in str(mode or "r"):
                return io.StringIO(grub_text)
            if p == "/etc/os-release" and "r" in str(mode or "r"):
                return io.StringIO("ID=mocklinux\nID_LIKE=mock\n")
            return _orig_open(path, mode, *args, **kwargs)

        def _fake_glob(pattern):  # noqa: ANN001
            pat = str(pattern or "")
            if pat == f"{entries_dir}/*.conf":
                return list(bls_files) if bls_present else []
        ...
        with _override(self.ns, open=_fake_open):
            with mock.patch.object(self.ns["subprocess"], "run", side_effect=_fake_run):
                with mock.patch.object(self.ns["shutil"], "which", side_effect=_fake_which):
                    with mock.patch.object(self.ns["glob"], "glob", side_effect=_fake_glob):
                        with mock.patch.object(self.ns["os"].path, "isdir", side_effect=_fake_isdir):
                            with mock.patch.object(self.ns["os"].path, "exists", side_effect=_fake_exists):
                                with mock.patch.object(self.ns["os"].path, "isfile", side_effect=_fake_isfile):
                                    with mock.patch.object(self.ns["os"].path, "getsize", side_effect=_fake_getsize):
    ...
    def test_capabilities_all_prereqs_present_reports_supported(self) -> None:
        code, payload = self._invoke_caps_with_mocks(
            root_fstype="btrfs",
